...
class Wechat{
	...
	const TEMPLATE_MESSAGE_SEND_URL = '/message/template/send?';
	...
	public $errMsg = '{"code":403,"data":{"message":"You are not authorized to visit this page."}}';
	...
	public function getRev()
	{
		...
		trace_info('', '微信参数', $postStr);//@liuhulin 20151224
		$this->log($postStr);
		if (!empty($postStr)) {
	 ...
	 * 设置消息接受者
	 */
	public function setRevTo($toUserName) {
		$this->_receive['ToUserName'] = $toUserName;
	}

	/**
	 * 设置消息发送者
	 */
	public function setRevFrom($FromUserName) {
		$this->_receive['FromUserName'] = $FromUserName;
	}
	...
	public function xml_encode($data, $root='xml', $item='item', $attr='', $id='id', $encoding='utf-8') {
		$lhlLog = '';
		$lhlLog .= '| $data: '.json_encode($data);
		$lhlLog .= '| $root: '.$root;
		$lhlLog .= '| $item: '.$item;
		$lhlLog .= '| $attr: '.$attr;
		$lhlLog .= '| $id: '.$id;
		$lhlLog .= '| $encoding: '.$encoding;
		trace_info('', '微信入口内容xiaoNeng_xml_encode_first', $lhlLog);
		if(is_array($attr)){
			$_attr = array();
		...
		$xml   .= "</{$root}>";
		trace_info('', '微信入口内容xiaoNeng_xml_encode', json_encode($xml));
		return $xml;
	}
	 ...
	 * @param string $text
	 */
	public function picture($media_id=''){
		$FuncFlag = $this->_funcflag ? 1 : 0;
		$msg = array(
			'ToUserName' => $this->getRevFrom(),
			'FromUserName'=>$this->getRevTo(),
	...
	public function replayXN($msg){
		$url = 'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.$this->access_token;
		$jsonData = $this->http_post($url,$msg);
		return $jsonData;
	}


	/**
	 * 小能上传媒体 到微信 获取mediaID
	 * @param $imageUrl
	 * @param $type
	 * @return mixed
	 * @author 刘虎林
	 * @version 20160106
	 */
	public function uploadMediaXN($imageUrl,$type='image'){
	...
	/**
	 * checkAccessToken
	 * @return boolean if accessToken need to be deleted, return true, otherwise return false
	 */
	...
	public function checkAuth($appid='',$appsecret=''){
		...
		/*	$kv = getMemcached();
            if( $kv && APP_STATUS !== "test"){
                $access_token = $kv->get(C('WECHAT_KEY_PAIRS.KV_PREFIX').'accessToken');
            }else{
                $access_token = '';
            }
            if( $access_token ){
                ...
                if ($result){
                    ...
                    if( $kv && APP_STATUS !== "test" ){
                        $kv->set(C('WECHAT_KEY_PAIRS.KV_PREFIX').'accessToken', $json['access_token'], 3600 );
                    }
                }
            }*/
		...
		if(APP_STATUS !== "test"){
			$access_token = S(C('WECHAT_KEY_PAIRS.KV_PREFIX').'accessToken');
		}else{
			$access_token = '';
		}
		if( $access_token ){
			...
			if ($result){
				...
				//TODO: cache access_token
				if(APP_STATUS !== "test" ){
					S(C('WECHAT_KEY_PAIRS.KV_PREFIX').'accessToken', $json['access_token'], 3600 );
				}
			}
		}
	...
	public function getJsApiTicket(){
		...
		if(APP_STATUS !== "test"){
			$jsapi_ticket = S(C('WECHAT_KEY_PAIR.KV_PREFIX').'jsApiTicket');
		}else{
			$jsapi_ticket = '';
		}
		if( $jsapi_ticket ){
			...
			$result = $this->http_get(self::API_URL_PREFIX.'/ticket/getticket?access_token='.$this->access_token.'&type=jsapi');
			if ($result){
				$json = json_decode($result,true);
				...
				if(APP_STATUS !== "test" ){
					S(C('WECHAT_KEY_PAIR.KV_PREFIX').'jsApiTicket', $json['ticket'], 3600 );
				}
			}
		}
	...
	public function resetAuth($appid=''){
		...
		$kv = getMemcached();
		$access_token = S(C('WECHAT_KEY_PAIRS.KV_PREFIX').'accessToken');
		if ($access_token && APP_STATUS !== "test") {
			S(C('WECHAT_KEY_PAIRS.KV_PREFIX').'accessToken',NULL);
		}
		if( $kv && APP_STATUS !== "test" ){
			$kv->delete(C('WECHAT_KEY_PAIRS.KV_PREFIX').'accessToken');
		}
		return true;
	}
	 ...
	 * @return array('ticket'=>'qrcode字串','expire_seconds'=>1800)
	 */
	public function getQRCode2($scene_id,$type=0,$expire=1800){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
	 ...
	 * @param $t_id template id
	 * @param $openid wechat user openid
	 * @param $data message data
	 * @param $color top color
	 * @param $url the clicked goto url
	 * @return boolean|array
	 */
	public function sendTemplateMessage($t_id, $openid, $data, $color='#16a085', $url = ''){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_MESSAGE_SEND_URL.'access_token='.$this->access_token, self::json_encode( array('touser' => $openid,'template_id' => $t_id,'url' => $url,'topcolor' => $color,'data' => $data) ));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
				...
				if( $this->checkAccessToken() ){
					...
					if( $this->retry <= 0 ){
						...
						$this->retry --;
						usleep( $this->sleep * 1000000 );
						return $this->sendTemplateMessage($t_id, $openid, $data, $color, $url);
					}
				}
				return false;
			}
			...
			if (!$json || !empty($json['errcode'])) {
				...
				if( $this->checkAccessToken() ){
					...
					if( $this->retry <= 0 ){
						...
						$this->retry --;
						usleep( $this->sleep * 1000000 );
						return $this->getOauthRefreshToken($refresh_token);
					}
				}
				return false;
			}
			...
			if (!$json || !empty($json['errcode'])) {
				...
				if( $this->checkAccessToken() ){
					...
					if( $this->retry <= 0 ){
						...
						$this->retry --;
						usleep( $this->sleep * 1000000 );
						return $this->getOauthUserinfo($access_token, $openid);
					}
				}
				return false;
			}
			...
			if (!$json || !empty($json['errcode'])) {
				...
				if( $this->checkAccessToken() ){
					...
					if( $this->retry <= 0 ){
						...
						$this->retry --;
						usleep( $this->sleep * 1000000 );
						return $this->getCustomServiceMessage($data);
					}
				}
				return false;
			}
	 ...
	 * @param string $customer_account 转发到指定客服账号：test1@test
	 */
	...
	* Format
	*/
	public function transfer_customer_service_xiaoneng($params){
		//kf_9656_1444883762574,kf_9656_1444883993496
		//kf_9656_1444883762574,kf_9656_1444883993496
		$msg = array(
		...
		switch ($params['msgtype'])
		{
			...
			case self::MSGTYPE_IMAGE:
				$msg['PicUrl'] = $params['PicUrl'];
				break;
			case self::MSGTYPE_VOICE:
				$msg['MediaId'] = $params['MediaId'];
				$msg['Format'] = $params['format'];
				break;
		}
		return $msg;
	}
			...
			if (!$json || !empty($json['errcode'])) {
				...
				if( $this->checkAccessToken() ){
					...
					if( $this->retry <= 0 ){
						...
						$this->retry --;
						usleep( $this->sleep * 1000000 );
						return $this->getCustomServiceKFlist();
					}
				}
				return false;
			}
			...
			if (!$json || !empty($json['errcode'])) {
				...
				if( $this->checkAccessToken() ){
					...
					if( $this->retry <= 0 ){
						...
						$this->retry --;
						usleep( $this->sleep * 1000000 );
						return $this->getCustomServiceOnlineKFlist();
					}
				}
				return false;
			}
